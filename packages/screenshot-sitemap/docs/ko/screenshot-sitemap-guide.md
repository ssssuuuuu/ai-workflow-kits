...
스크린샷과 영상 프레임을 재료로 **종합 분석 보고서** 하나를 만드는
워크플로우입니다. 보고서는 depth가 맞춰진 사이트맵 트리(뼈대), 최신
웹트렌드 감사, 화면에서 역추출한 **디자인 시스템**, **UI/UX 가이드**,
그리고 (HTML 문서 제공 시) 컨설팅 섹션과 통합 로드맵을 포괄합니다.

PPT로 녹화한 웹/앱 테스트 영상도 입력으로 받아 1~2초 간격으로 프레임을
추출하고 중복 화면은 제거한 뒤 스크린샷과 동일하게 분석합니다. 페이지별
HTML 속성/태그가 정리된 문서를 함께 주면, 비주얼 디자인과 마크업 품질을
교차 검증합니다.
...
| Design system audit | `claude/skills/screenshot-sitemap/references/design-system-audit.md` | 관찰 기반 디자인 토큰 추출(컬러/타이포/셰이프), 컴포넌트×화면 매트릭스, 드리프트 진단, 8항목 UI/UX 평가 루브릭과 Keep/Change/Codify 가이드 작성법 |
...
6. **디자인 시스템은 관찰이지 발명이 아니다.** 토큰·컴포넌트는 화면에
   실제로 보이는 것만 기록하고, 각 항목마다 샘플링한 화면을 인용한다.
   화면 간 스타일 불일치(드리프트)는 그 자체가 핵심 finding이며, UI/UX
   가이드의 모든 Keep/Change/Codify 항목에도 근거 화면이 붙어야 한다.
7. **컨설팅 결론은 반드시 양쪽 근거를 쌍으로 인용한다.** 스크린샷의 시각적
...
9. `design-system-audit.md` 기준으로 관찰된 디자인 시스템 추출: 토큰
   표(각 토큰마다 샘플링한 화면 인용), 컴포넌트×화면 매트릭스(캡처 안 된
   상태는 `not observed`로 구분), 드리프트 진단, 성숙도 판정
   (`consistent | drifting | ad hoc`)
10. UI/UX 가이드 작성: 8항목 루브릭(내비/상태 가시성/일관성/오류 복구/
    인지 부담/효율/시각 접근성/감성 품질)으로 플로우를 평가하고
    **Keep(유지·성문화) / Change(위반+화면+수정안) / Codify(사실상 표준을
    규칙으로)** 3버킷 가이드라인 작성 — 모든 항목에 화면 인용
11. HTML 문서가 있으면 `html-signal-checklist.md` 기준으로 교차 검증 후
    컨설팅 섹션 생성
12. 종합 보고서 작성: 경영 요약(맨 앞) → 인벤토리 요약(영상별
    추출/제거/채택 수 포함) → 트리 → 노드별 표 → 충돌/저신뢰 항목 →
    트렌드 정합성 요약 → 디자인 시스템(관찰) → UI/UX 가이드 →
    (컨설팅 모드) 스코어카드·우선순위 findings → 전 섹션을 합친
    **통합 로드맵**(quick win 우선)
