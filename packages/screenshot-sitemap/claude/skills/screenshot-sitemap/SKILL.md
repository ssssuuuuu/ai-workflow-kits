...
description: >
  ...
  depth/hierarchy analysis of the screens. Also use it when those
  screenshots come with documents listing each page's HTML attributes and
  tags, and the user wants a consulting report that cross-checks visual
  design against markup quality. Triggers include "이 스크린샷들로
  사이트맵 만들어줘", "화면들 depth 맞춰서 트리로 정리해줘", "html 태그/속성
  ...
  coverage, or a design+markup consulting report from a folder of images.
---
...
1. **Input summary** — folder, screenshot count, doc count, unmatched
   pages, any unreadable/excluded files
2. **Depth-normalized tree** — Markdown tree + Mermaid diagram
3. **Per-node table** — depth, file, title, page type, trend tags, HTML
   signal summary, evidence
...
`html-signal-analyst` in batches the same way. Both agents only extract
...
```text
Input: <folder>, <N> screenshots (<M> excluded/unreadable), <D> html docs
