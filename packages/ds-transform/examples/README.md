...
- `datalist-composition/`: a fictional to-do list screen transformed into a `DataList`/`DataListItem` component family, demonstrating the composition layer — how atoms (title/description/meta/label/checkbox/icon) become named slots of a molecule component, how a `region` (left/right) layout variant replaces two separate components, and how components assemble into a screen. See section 18 of `docs/design-system-methodology.md`.
